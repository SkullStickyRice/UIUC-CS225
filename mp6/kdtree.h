...
class KDTree
{
  ...
  private:
	...
	int distance(const Point<Dim>& p, const Point<Dim>& q) const;
	void KDTree_helper(int a, int b, int dimension);
	void quickSelect(int left, int right, int dimension, int mid);
	int partition(int left, int right, int pivotIndex, int dimension);
	Point<Dim> NN_helper(int left, int right, int dimension, const Point<Dim>& query) const;
