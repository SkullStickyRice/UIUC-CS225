...
RGBAPixel gradientColorPicker::operator()(int x, int y)
{
	...
	d = abs(middleX - x) + abs(middleY - y);
	if (d < speed) {
		//redFill 
		color.red = color1.red - (d*color1.red/speed) + (d*color2.red/speed);
		color.green = color1.green - (d*color1.green/speed) + (d*color2.green/speed);
		color.blue = color1.blue - (d*color1.blue/speed) + (d*color2.blue/speed);
